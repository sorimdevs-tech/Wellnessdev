...
export const apiClient = {
  ...
  async rejectAppointment(id: string, reason?: string) {
    return fetchAPI(`/appointments/${id}/reject`, {
      method: "PUT",
...
export const fhirApi = {
  ...
  async getCapabilityStatement() {
    return fetchAPI("/fhir");
  },

  // ---- Patient ----
  async getPatients(params?: { name?: string; gender?: string; _count?: number }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/Patient${query ? `?${query}` : ""}`);
  },

  async getPatientById(id: string) {
    return fetchAPI(`/fhir/Patient/${id}`);
  },

  // ---- Person ----
  async getPersons(params?: { name?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/Person${query ? `?${query}` : ""}`);
  },

  // ---- Practitioner ----
  async getPractitioners(params?: { specialty?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/Practitioner${query ? `?${query}` : ""}`);
  },

  // ---- PractitionerRole (Doctors + Roles) ----
  async getPractitionerRoles(params?: { role?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/PractitionerRole${query ? `?${query}` : ""}`);
  },

  // ---- RelatedPerson ----
  async getRelatedPersons() {
    return fetchAPI("/fhir/RelatedPerson");
  },

  // ---- Group ----
  async getGroups() {
    return fetchAPI("/fhir/Group");
  },

  // ---- Encounter ----
  async getEncounters() {
    return fetchAPI("/fhir/Encounter");
  },

  // ---- EpisodeOfCare ----
  async getEpisodesOfCare() {
    return fetchAPI("/fhir/EpisodeOfCare");
  },

  // ---- Appointment ----
  async getAppointments(params?: { patient?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/Appointment${query ? `?${query}` : ""}`);
  },

  // ---- AppointmentResponse ----
  async getAppointmentResponses() {
    return fetchAPI("/fhir/AppointmentResponse");
  },

  // ---- Schedule ----
  async getSchedules() {
    return fetchAPI("/fhir/Schedule");
  },

  // ---- Slot ----
  async getSlots() {
    return fetchAPI("/fhir/Slot");
  },

  // ---- Task ----
  async getTasks() {
    return fetchAPI("/fhir/Task");
  },

  // ---- ServiceRequest ----
  async getServiceRequests() {
    return fetchAPI("/fhir/ServiceRequest");
  },

  // ---- Condition ----
  async getConditions(params?: { patient?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/Condition${query ? `?${query}` : ""}`);
  },

  // ---- Procedure ----
  async getProcedures() {
    return fetchAPI("/fhir/Procedure");
  },

  // ---- Observation ----
  async getObservations(params?: { patient?: string; code?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/Observation${query ? `?${query}` : ""}`);
  },

  // ---- DiagnosticReport ----
  async getDiagnosticReports() {
    return fetchAPI("/fhir/DiagnosticReport");
  },

  // ---- Organization ----
  async getOrganizations() {
    return fetchAPI("/fhir/Organization");
  },

  // ---- Location ----
  async getLocations() {
    return fetchAPI("/fhir/Location");
  },

  // ---- HealthcareService ----
  async getHealthcareServices() {
    return fetchAPI("/fhir/HealthcareService");
  },

  // ---- Medication ----
  async getMedications() {
    return fetchAPI("/fhir/Medication");
  },

  // ---- MedicationRequest ----
  async getMedicationRequests(params?: { patient?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return fetchAPI(`/fhir/MedicationRequest${query ? `?${query}` : ""}`);
  },
};